...
namespace BoxSocial.Internals
{
    ...
    public class StatusMessage : NumberedItem, ICommentableItem, IPermissibleItem, ILikeableItem, ISearchableItem, IShareableItem, IActionableItem
    {
        ...
        public static void NotifyStatusMessageComment(Core core, Job job)
        {
            ...
            Template emailTemplate = new Template(ae.Assembly, core.TemplateEmailPath, "email_status_comment");
            ...
            emailTemplate.Parse("STATUS_OWNER_DISPLAYNAME_OWNERSHIP", ev.Owner.DisplayNameOwnership);
            emailTemplate.Parse("U_VIEW_STATUS", core.Hyperlink.StripSid(core.Hyperlink.AppendAbsoluteSid(comment.BuildUri(ev))));

            if (ev.Owner is User && (!comment.OwnerKey.Equals(ev.OwnerKey)))
            {
                ae.SendNotification(core, (User)ev.Owner, ev.ItemKey, string.Format("[user]{0}[/user] commented on your [iurl=\"{1}\"]status[/iurl]", comment.OwnerKey.Id, comment.BuildUri(ev)), string.Empty, emailTemplate);
            }

            ae.SendNotification(core, comment.OwnerKey, ev.ItemKey, string.Format("[user]{0}[/user] commented on [user]{2}[/user] [iurl=\"{1}\"]status[/iurl]", comment.OwnerKey.Id, comment.BuildUri(ev), ev.OwnerKey.Id), string.Empty, emailTemplate);
        }
