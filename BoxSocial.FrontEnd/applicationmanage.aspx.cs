...
{
    public partial class applicationmanage : APage
    {
        ...
        private void loadSubModulesFromAssembly(Account accountObject, Assembly assembly, string module)
        {
            ...
            foreach (Type type in types)
            {
                ...
                {
                    AccountSubModule newModule = System.Activator.CreateInstance(type, new object[] { Core, AnApplication }) as AccountSubModule;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            template.Parse("ACCOUNT_TITLE", "Application Control Panel");
            template.Parse("PRIMITIVE_TITLE", AnApplication.DisplayName);
            template.Parse("U_PRIMITIVE", AnApplication.Uri);

            Account accountObject = new Account(Core);
            loadModules(accountObject, BoxSocial.Internals.Application.GetModuleApplications(core, AnApplication), module);
            ...
            foreach (AccountModule accountModule in accountModules)
            {
                ...
                {
                    modulesVariableCollection.Parse("URI", AnApplication.AccountUriStub);
                }
                else
                {
                    modulesVariableCollection.Parse("URI", AnApplication.AccountUriStub + accountModule.Key);
                }
